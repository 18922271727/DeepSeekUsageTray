...
| `CsdnCli.cs` | 命令行：`csdn check / login / logout / publish / update / list / view / upload`；`publish/update` 默认自动上传内嵌的鲸鱼封面（`logo-gray-blue.png`），可用 `--cover <封面图>` 覆盖、`--cover none` 关闭 |
...
- [x] 默认封面：`logo-gray-blue.png`（鲸鱼+两行文字）内嵌为发布默认封面，`--cover` 可覆盖（已实测）；
